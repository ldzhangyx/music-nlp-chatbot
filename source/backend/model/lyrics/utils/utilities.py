...
def load_dataset(dataset_path):
    ...
    with open(dataset_path, 'r', encoding='utf-8') as f:
        ...
        for line in tqdm(f):
            # for lyrics > 1024
            # Output      (tags,   keywords,   lyrics)
            output.append((line[0], line[1], line[2][:256]))
    return output
...
def format_n_tokenize_data(raw_dataset, enc):
    ...
    formatted_data = []
    for tags, keywords, lyrics in raw_dataset:
        tag = [spe["[s:tags]"]] + enc.encode(tags) + [spe["[e:tags]"]]
        keyword = [spe["[s:keywords]"]] + enc.encode(keywords) + [spe["[e:keywords]"]]
        lyric = [spe["[s:lyrics]"]] + enc.encode(lyrics) + [spe["[e:lyrics]"]]

        formatted_data.append((tag, keyword, lyric))

    print("The exceeding in length inputs are removed from the dataset.")
    return formatted_data


def construct_input(formatted_data, device, max_input_len=1024):
    """
    ...
    for tags, keywords, lyrics in formatted_data:
        # 1) Prepare input partitions, i.e., token type ids & position ids
        # Token type ids, alternatively called segment ids
        tag_seg = list([1] * len(tags))
        key_seg = list([2] * len(keywords))
        lyr_seg = list([3] * len(lyrics))
        ...
        # 10% of dropping the individual features
            ...
            if np.random.rand() > 0.1:
                tokens_subset += tags
                segment_subset += tag_seg

            if np.random.rand() > 0.1:
                tokens_subset += keywords
                segment_subset += key_seg
